...
const Phoniex = () => {
  ...
  const generateParkingData = () => {
    ...
    levels.forEach((level) => {
      sections.forEach((section) => {
        for (let i = 1; i <= 8; i++) {
          const spotNumber = `${level}${section}${i.toString().padStart(2, "0")}`
          spots.push({
            id: `${level}-${section}-${i}`,
            number: spotNumber,
            status: "empty",
  ...
  // Fetch occupancy from backend and apply to parking data (Phoenix)
  useEffect(() => {
    const fetchOccupancy = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/mall/occupancy?mall=Phoenix", {
          headers: { Accept: "application/json" },
        })
        if (!response.ok) return
        const records = await response.json()
        const normalized = (records || [])
          .filter((r) => (r.mall || "").toLowerCase() === "phoenix")
        ...
        normalized.forEach((code) => {
          ...
          const current = levelSectionToMax.get(key) || 8
          if (idx > current) levelSectionToMax.set(key, idx)
        })

        // Build a new parking map that at least covers DB slots
        const derived = []
        const allLevels = new Set(["B1", "B2"]) // defaults
        const allSections = new Set(["A", "B", "C", "D", "E", "F", "G", "H"]) // defaults
        normalized.forEach((code) => {
          const m = code.match(/^(B\d)([A-Z])\d{2}$/)
          if (m) {
            allLevels.add(m[1])
        ...
        Array.from(allLevels).forEach((level) => {
          Array.from(allSections).forEach((section) => {
            ...
            const maxIdx = levelSectionToMax.get(key) || 8
  ...
  return (
    <div style={{ backgroundColor: "#1a1a1a", minHeight: "100vh", color: "#ffffff" }}>
      <style jsx>{`
        ...
        .level-button {
          background-color: ${currentLevel === "B1" ? "#ffd700" : "#333"};
          color: ${currentLevel === "B1" ? "#000000" : "#ffffff"};
          border: none;
          padding: 12px 24px;
          border-radius: 8px;
          font-size: 16px;
      ...
      <div className="header">
        ...
        <div className="header-info">
          <h1>Phoenix mall parking</h1>
          <p>Real-time occupancy view for Phoenix Mall</p>
        </div>
      </div>
      ...
      <div className="level-navigation">
        <button
          className={`level-button ${currentLevel === "B1" ? "active" : ""}`}
          onClick={() => setCurrentLevel("B1")}
        >
          Basement 1 (B1)
        </button>
        <button
          className={`level-button ${currentLevel === "B2" ? "active" : ""}`}
      ...
      <div className="parking-container">
        ...
        <div className="parking-levels" ref={scrollContainerRef}>
          <div className="level-rows">
            <div className="level-row">{["A","B","C","D"].map((section) => renderParkingSection(section))}</div>
            <div className="level-row">{["E","F","G","H"].map((section) => renderParkingSection(section))}</div>
          </div>
        </div>
      </div>
    </div>
