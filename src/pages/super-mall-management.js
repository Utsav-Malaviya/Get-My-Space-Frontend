...
const SuperMallManagement = () => {
  ...
  const fetchMalls = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:5000/api/super/malls', {
        headers: {
  ...
  const handleUpdateMall = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch(`http://localhost:5000/api/super/malls/${editingMall._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleDeleteMall = async (mallId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/super/malls/${mallId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleToggleLive = async (mallId) => {
    try {
      const response = await fetch(`http://localhost:5000/api/super/malls/${mallId}/toggle-live`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleUpdateOffset = async (mallId, offsetData) => {
    try {
      const response = await fetch(`http://localhost:5000/api/super/malls/${mallId}/offset`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
